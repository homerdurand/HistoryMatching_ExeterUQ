...
ImplausibilityMOGP <- function(NewData, Emulator, Discrepancy, Obs, ObsErr, parallel=TRUE){
  X <- torch$tensor(np$array(NewData), dtype=torch$float)
  pred <- Emulator$emulator$predict(X)
  sapply(1:Emulator$n_emulators, function(k) abs(Obs[k] - pred[[k]]$mean$detach()$numpy())/sqrt(pred[[k]]$variance$detach()$numpy() + Discrepancy[k] + ObsErr[k]))

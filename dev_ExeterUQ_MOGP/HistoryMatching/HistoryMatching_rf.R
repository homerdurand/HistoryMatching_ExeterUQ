...
ImplausibilityMOGP <- function(NewData, Emulator, Discrepancy, Obs, ObsErr, parallel=TRUE){
  ...
  if(parallel){
    ...
    preds <- lapply(1:Emulator$n_emulators, function(u) randomForestInfJack(Emulator$morf[[u]], NewData, calibrate = TRUE))
    future_sapply(1:Emulator$n_emulators, function(k) abs(tObs[[k]] - preds[[k]]$y.hat)/sqrt(preds[[k]]$var.hat + tDisc[k] + tObsErr[k]), future.seed=NULL)

  }
  else{
      stds <- sapply(1:Emulator$n_emulators, function(u) predict(Emulator$morf[[u]], Xp, what=sd))
      means <- sapply(1:Emulator$n_emulators, function(u) predict(Emulator$morf[[u]], Xp, what=mean))
      sapply(1:Emulator$n_emulators, function(k) abs(tObs[[k]] - means[,k])/sqrt(stds[,k] + tDisc[k] + tObsErr[k]))
  }
}

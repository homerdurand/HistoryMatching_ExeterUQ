packages <- c('reticulate','pracma', 'invgamma', 'GenSA', 'far', 'fields', 'lhs', 'maps', 'mco', 'mvtnorm', 'ncdf4', 'parallel', 'shape', 'tensor', 'withr', 'loo','MASS', 'neuralnet', 'randomForest')
...
torch <- import('torch')
bnn <- import("/home/homer/Documents/Locean/HistoryMatching/bayesian-neural-network-pytorch")
source_python("../BNNemulators.py")
...
BuildNewEmulator <- function(tData, 
                              outputs_dim,
  ...
  if(is.null(outputs_dim))
    outputs_dim <- length(names(tData)) - lastCand
  if(!(outputs_dim == length(names(tData)) - lastCand)){
    tData <- tData[,c(1:lastCand,(lastCand+1):(lastCand+outputs_dim))]
  }
  ...
  #Set inputs and targets to torch tensors
  inputsTens <- torch$tensor(inputs, dtype=torch$float)
  targetsTens <- torch$tensor(targets, dtype=torch$float)

  ###Establish and fit the MOGP on residuals###
  #Set inputs to torch tensor
  bbn_emulator = BNNemulator(d_in, d_tar)
  bbn_emulator$fit(inputsTens, targetsTens)

  print("BNN fitted !")

  return(list(emulator = bbn_emulator,
              target_dim = d_tar))
